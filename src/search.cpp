...
uint64_t Search::perft(Position& pos, Depth depth) {
...
#endif
  {
...
#endif
          cnt = leaf ? MoveList<MV_LEGAL>(pos).size() : perft<false>(pos, depth - ONE_PLY);
          nodes += cnt;
      ...
      if (Root)
#ifdef NANOHA
		  sync_cout << UCI::move(ml.move()) << ": " << cnt << sync_endl;
#else
...
void MainThread::search() {
  ...
  int contempt = Options["Contempt"] * PawnValueMidgame / 100; // From centipawns
...
#ifdef NANOHA
  bool SentBestmove = false;
  // 入玉勝ち宣言できるか？
  if (rootPos.IsKachi(us) != false) {
	  SentBestmove = true;
	  sync_cout << "bestmove win";
  }
...
#endif
#ifdef USAPYON2
  // 投了するべき時にどこかからillegal moveを持って来る現象の対策
  // まだ合っているかどうか自信なし…。
  if (!SentBestmove) {
	  if (rootMoves.empty()) {
...
#else
      sync_cout << "info depth 0 score "
                << UCI::value(rootPos.in_check() ? -VALUE_MATE : VALUE_DRAW)
                << sync_endl;
#endif
  }
  else
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!RootNode)
    {
        ...
        if (Signals.stop.load(std::memory_order_relaxed) || pos.is_draw(repeat_check) || ss->ply >= MAX_PLY)
		...
		)
    {
        ...
        Depth R = ((823 + 67 * depth) / 256 + std::min((eval - beta) / PawnValueMidgame, 3)) * ONE_PLY;
...
moves_loop: // When in check search starts from here
	...
	if (prevSq==0 || (ss - 1)->currentMove==MOVE_NULL) {
...
#ifndef NANOHA
		  // 諦めた。
		  // 方法としては、「fromの位置に利きがある場合、toの駒を取り除き、fromの位置に駒を打つ手でsee()して、toの駒を戻して、see()の結果でrを減らす。」from かず＠なのは
		  // toの駒を取り除く際に利きの更新が必要、toの駒を戻す時にも利きの更新が必要、で面倒…。
		  // fromの位置に利きがあるかどうかに関しても、toの駒を取り除いてから利きを見る必要がある。
		  if (   r
			  // 将棋的には常にNORMAL?
			  && type_of(move) == NORMAL
			  // ToDo: from,toが逆になっているのだが、直前でPAWNを排除しているので、
			  // 逆にしてもチェスの場合には正しい動きになる…のだが、将棋では成立しない。
			  && type_of(pos.piece_on(to_sq(move))) != PAWN
			  && pos.see(make_move(to_sq(move), from_sq(move))) < VALUE_ZERO)

			  // ToDO: 上記コメント踏まえて、このやり方は多分間違っている…のだが、どうするべきか？
...
#else
	int repeat_check = 0;
	if (pos.is_draw(repeat_check) || ss->ply >= MAX_PLY) {
		return ss->ply >= MAX_PLY && !InCheck ? evaluate(pos)
			: DrawValue[pos.side_to_move()];
	}
#endif
...
//      pos.do_move(move, st, givesCheck);
  ...
  Move Skill::pick_best(size_t multiPV) {
    ...
    int delta = std::min(topScore - rootMoves[multiPV - 1].score, PawnValueMidgame);
